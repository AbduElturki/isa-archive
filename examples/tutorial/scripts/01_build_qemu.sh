#!/usr/bin/env bash
#
# 01_build_qemu.sh — Step 1: YAML → qemu-system-<isa>
#
# Generates a custom QEMU CPU target from the ISA YAML, clones QEMU, integrates
# the generated files, and builds the simulator binary. Defaults to the part-4
# pico32 snapshot (see common.sh to point it elsewhere).
#
# Usage:
#   bash 01_build_qemu.sh [BUILD_DIR]
#   BUILD_DIR=/tmp/my-pico32 bash 01_build_qemu.sh
#
# After building:
#   $BUILD_DIR/qemu/build/qemu-system-pico32 --version

set -euo pipefail
source "$(dirname "$0")/common.sh"
[ -n "${1:-}" ] && BUILD_DIR="$1" && QEMU_BUILD_DIR="$BUILD_DIR/qemu" && QEMU_SRC="$QEMU_BUILD_DIR/qemu-src" && QEMU_BIN="$QEMU_BUILD_DIR/build/qemu-system-${ISA_NAME}"
...
echo "[4/5] Configuring QEMU ($QEMU_TARGET only) ..."
mkdir -p "$QEMU_BUILD"

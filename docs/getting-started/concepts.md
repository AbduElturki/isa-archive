...
```
YAML manifests  ──▶  validated model  ──▶  generators  ──▶  artifacts

isa.yaml             every field checked     qemu          a CPU for QEMU
schemas, instrs,     before anything is      llvm          a clang backend
operands, enums      generated               asm           an assembler
                                             c / rust      intrinsics headers
                                             verilog       RTL skeletons
                                             docs          a reference manual
```
...
read it; [part 3 of the tutorial](../tutorial/03-compiling-c.md) walks the
whole journey from "simulates" to "compiles C".

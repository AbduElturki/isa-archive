...
Finished state: [`examples/tutorial/pico32-part4/`](../../examples/tutorial/pico32-part4/).
...
(Source and the full build commands are in the snapshot's `programs/` and in
[the compiler build guide](../compiler/build-and-use.md).) Run it:
...
[`-t c` / `-t rust`](../targets/intrinsics.md) emit typed headers — inline-asm
wrappers, operand structs, CSR accessors:
...
static inline uint32_t isa_archive_MUL() { ... }
```

## Ship a manual

[`-t docs`](../targets/reference-manuals.md) turns the same manifests — and
your `description:` fields — into a reference manual:
...
Everything so far was software. The [`verilog` target](../targets/verilog.md)
turns your ISA + a [uArch manifest](../yaml/uarch.md) into SystemVerilog. A
minimal chassis, `uarch.yaml`:
...
not a finished core (see [the Verilog target's boundaries](../targets/verilog.md#what-it-is--and-isnt)).
...
- **[`examples/showcase`](../examples.md#showcase--multiple-register-classes-and-wide-instructions)** —
  a second register class (floating point) and 64-bit instruction words.
- **[`examples/npu-probe`](../examples.md#npu-probe--the-not-a-cpu)** — what
  this looks like for an accelerator that *doesn't* run C (`kernel-only`
  profile, 128-bit vectors).
- **The [manifest reference](../yaml/README.md)** — your companion now that
  you know the shape of the thing.

...
Finished state: [`examples/tutorial/pico32-part1/`](../../examples/tutorial/pico32-part1/).
...
[the schema reference](../yaml/schemas.md#split-immediates).
...
The full language: [the behavior DSL](../yaml/behavior.md).
...
(That's exactly what `examples/demo/01_build_qemu.sh` automates for the
RISC-V example.) Coffee. When ninja finishes:
...
Assemble with the [generated assembler](../targets/assembler.md) and run:
...
[**Part 2: a real instruction set →**](02-a-real-instruction-set.md)

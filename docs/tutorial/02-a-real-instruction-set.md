...
Finished state: [`examples/tutorial/pico32-part2/`](../../examples/tutorial/pico32-part2/).
...
[Constants and Enums](../yaml/types.md) in a `constants.yaml`:
...
scatters across four fields — same trick, see the snapshot's `schemas.yaml`);
...
Second program — `sum.s` (in the snapshot's `programs/`) stores 1…5 to RAM,
...
[**Part 3: compiling C →**](03-compiling-c.md)
